...
router.get('/', async (req, res) => {
  try {
    ...
    res.json(result.rows); // returns all expenses
  } catch (error) {
...
router.post('/', [
  ...
  body('expense_type').notEmpty().withMessage('Expense type is required'),
...
async (req, res) => {
  ...
  const { status, user_id, expense_type, description, amount } = req.body;
  ...
  try {
    ...
    const query = `
      INSERT INTO expenses (status, user_id, expense_type, description, amount)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING *;
    `;

    // Execute the query and pass in the corresponding values
    const result = await pool.query(query, [status, user_id, expense_type, description, amount]);
...
router.put('/:id', [
  ...
  body('expense_type').optional().notEmpty().withMessage('Expense type cannot be empty'),
